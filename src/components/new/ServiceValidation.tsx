...
const ServiceValidation: React.FC = () => {
  ...
  const handleValidateOTP = (otp: string) => {
    if (otp === "789012") {
      ...
      dispatch({ type: "VALIDATE_SERVICE", payload: true });
  ...
  const handleRejectValidation = () => {
    setValidationStatus("rejected");
    dispatch({ type: "VALIDATE_SERVICE", payload: false });
  };
  ...
  if (!state.patient || !state.selectedService || !state.consentObtained) {
    return (
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {validationStatus === "approved" ? (
        ...
        <OTPValidation
          ...
          description={`Please enter the 6-digit OTP sent to the technician to confirm service booking for ${state.selectedService.description}.`}
            ...
            <div className="bg-gray-50 p-4 rounded-md mb-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{state.patient.patientName}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Patient ID</p>
                  <p className="font-medium">{state.patient.patientId}</p>
                </div>
                <div>
